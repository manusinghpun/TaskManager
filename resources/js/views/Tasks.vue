  ...
  <div class="container-fluid">
    <div class="card border-left-primary shadow h-100 py-2">
      <div class="card-header d-flex">
        ...
        <button type="button" v-if="$store.state.profile.role == 'administrator'" class="btn btn-primary btn-sm ml-auto" v-on:click="showNewTaskModal">
      ...
      <div class="card-body">
        ...
        >
          ...
          <template #cell(pdc)="data">
            <small
              v-if="data.item.pdc == currentDate"
              class="text-warning"
            >{{ data.item.pdc | myDate }}</small>
            <small
              v-else-if="data.item.pdc < currentDate"
              class="text-danger"
            >{{ data.item.pdc | myDate }}</small>
            <small
              v-else-if="data.item.pdc < nextThreeDate && data.item.pdc > currentDate"
              class="text-primary"
            >{{ data.item.pdc | myDate }}</small>
            <small v-else>{{ data.item.pdc | myDate }}</small>
          </template>
          ...
          <template #cell(created_by)="data"> <small>{{ findUserName(data.item.created_by) }}</small> </template>

          <template #cell(action)="data">
            <button
              class="btn btn-primary btn-sm"
              v-if="$store.state.profile.role == 'administrator'"
              v-on:click="editTask(data.item)"
            >
            ...
            <button
              ...
              v-if="$store.state.profile.role == 'administrator'"
              v-on:click="deleteTask(data.item)"
            >
          ...
          <template #row-details="data">
            ...
            <b-card>
              ...
              <br />
              <!--//form add reply form-->
              <b-alert show variant="secondary" v-for="item in data.item.replies" :key="item.id">
                <b-row>
                  <b-col sm="2" class="text-sm-right">
              ...
              <b-button size="sm" variant="warning" @click="data.toggleDetails">Hide Details</b-button>
            </b-card>
          </template>
        </b-table>
        <b-pagination
    ...
    <b-modal ref="editTaskModal" hide-footer title="Update task form">
      <div class="d-block">
        <form v-on:submit.prevent="updateTask">
          ...
          <div class="from-group">
            <label for="assign_to">Assign Task To <code> (Add User)</code></label>
            <multiselect
...
export default {
  ...
  data() {
    return {
      ...
      nextThreeDate: moment().add(3, "days").format("YYYY-MM-DD"),
    };
  },
