<template>
    <div class="container-fluid">
            ...
            <div class="card-body">
                <div class="row no-gutters align-items-center">
                    <table class="table">
                        ...
                        <tbody>
                            <tr v-for="(category, index) in categories" :key="index" v-bind:style="{color: category.color}">
                                <td>{{ index + 1 }}</td>
                                <td>{{ category.name }}</td>
        ...
        <b-modal ref="newCategoryModal" hide-footer title="Add category form">
            <div class="d-block">
                <form v-on:submit.prevent="createCategory">
                    ...
                    <div class="text-right">
                        <button type="button" class="btn btn-warning" v-on:click="hideNewCategoryModal">
                            Cancel
                        </button>
        ...
        <b-modal ref="editCategoryModal" hide-footer  title="Update category form" >
            <div class="d-block">
                <form v-on:submit.prevent="updateCategory">
                    ...
                    <div class="form-group">
                        <label for="color">Choose Color</label>
                        <input  type="color"  v-model="editCategoryData.color"  class="form-control"
                            name="color" id="color" placeholder="Enter category color" />
                        <div class="invalid-feedback" v-if="errors.color">
                            {{ errors.color[0] }}
                        </div>
                    </div>
                    <hr />
                    <div class="text-right">
                        <button type="button" class="btn btn-warning"  v-on:click="hideEditCategoryModal" >
                            Cancel
                        </button>
                        <button type="submit" class="btn btn-primary">
...
export default {
    ...
    methods: {
        ...
        deleteCategory: async function(category) {
            this.$confirm(`Are you sure you want to delete ${category.name}`).then(async () => {
                try {
        ...
        updateCategory: async function() {
            try {
                ...
                formData.append("color", this.editCategoryData.color);
